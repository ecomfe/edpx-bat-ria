...
function produce(projectInfo, type, pagePath, entryName, options) {
    if (type === 'action') {
        type = 'base';
    }
    ...
    var templateClasses = [];
    templateClasses.push(actionSegs.join('-').toLowerCase().replace(/_/g, '-'));
    templateClasses = templateClasses.join(' ');
    var styleClass = actionSegs.join('-').toLowerCase().replace(/_/g, '-');
    ...
    var fullModule = (entryName ? entryName + '/' : '') + action;
    ...
    readPath(pagePath, function (pagePath) {
        ...
        if (fs.existsSync(entryDir)) {
            ...
            readEntry(entries, entryName, function (entry)  {
                produce(projectInfo, type, pagePath, entry, options);
            });
        }
        else {
            produce(projectInfo, type, pagePath, null, options);
        }
    });
};
