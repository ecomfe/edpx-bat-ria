...
module.exports = require('./generator')(
    function (projectInfo, options) {
        ...
        if (options.isCreateAuth && fs.existsSync(modulesJSONPath)) {
            var moduelsJSON = JSON.parse(readFile(modulesJSONPath));
            var urlAuthMap = {};
            ...
            // 把url算出来顺便去掉
            pluckUrlAuthMap(moduelsJSON, urlAuthMap);

            writeFile(path.resolve(projectDir, 'modules_tree.txt'), JSON.stringify(moduelsJSON, null, 4));
            ...
            writeFile(path.resolve(projectDir, 'auth.txt'), authTxt);
        ...
        buildTree(getModuleNames(modulePath), 'auth', modulePath, modulesTree);
        ...
        writeFile(modulesJSONPath, JSON.stringify(modulesTree, null, 4));
    }
);
